#!/usr/bin/env bash
# Capstone scaffolding across the tools. NOTE: the free toolchain (Yosys +
# Verilator) has no CDC analysis, so the lint fixture will NOT contain the CDC
# hazard — identifying it is a manual student deliverable, and this is stated in
# the brief rather than papered over.
set -euo pipefail
PACK_DIR="$(cd "$(dirname "$0")" && pwd)"; cd "$PACK_DIR"; mkdir -p fixtures build
SRCS="src/producer.v src/sync2.v src/consumer.v src/soc_top.v"

yosys -q -p "read_verilog $SRCS; hierarchy -top soc_top; proc; opt; write_json fixtures/netlist.json"

raw="$(verilator --lint-only -Wall --timing $SRCS 2>&1 || verilator --lint-only -Wall $SRCS 2>&1 || true)"
...
json.dump({"schema_version":1,"engine":"verilator","note":"no CDC analysis — free toolchain has no CDC engine","violations":v},open("fixtures/lint.json","w"),indent=2); open("fixtures/lint.json","a").write("\n")
print("lint.json: %d findings (none are CDC)"%len(v))
'
iverilog -g2012 -o build/sim.vvp tb/tb_soc.v $SRCS
...
rep={"schema_version":1,"suite":"soc_cdc","tests":t,"summary":{"total":len(t),"passed":sum(x["outcome"]=="pass" for x in t),"failed":sum(x["outcome"]=="fail" for x in t)}}
